...
describe('Wait tests', () => {
    it('should wait for enabled', () => {
        ...
        expect(dynamicControlsPage.inputEnabledTextBox.isEnabled()).to.be.equal(true)
    ...
    it('should wait for input field to be disabled', () => {
        dynamicControlsPage.clickEnabledButton()
        dynamicControlsPage.waitWhileButtonIsDisplayed()

        expect(dynamicControlsPage.inputEnabledTextBox.isEnabled()).to.be.equal(false)
    })
})